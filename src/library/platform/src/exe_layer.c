...
opcode_t decode_opcode(uint16_t opcode){
	...
	uint8_t nibble1 = ((opcode & N1_MASK) >> 12 );
	uint8_t nibble2 = ((opcode & N2_MASK) >> 8);
	...
	switch (nibble1){
		...
		case 0: 
				switch (nibble2) {
					...
					case 3:
							result.id = 2;
							uint8_t nibble3 = ((opcode & N3_MASK) >> 4);
							result.reg1 = nibble3;
							uint8_t nibble4 = ((opcode & N4_MASK));
							result.reg2 = nibble4;
							break;
		...
		case 7:
				...
				result.reg1 = ((opcode & N2_MASK)>>8);
				result.value = ((opcode & B2_MASK));
				break;
